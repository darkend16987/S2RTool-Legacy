...
document.addEventListener('DOMContentLoaded', () => {
    debugLog('🚀 S2R Tool v3.1 initialized');
    loadAspectRatios();
    setupEventListeners();
...
// NOTE: optimizeImageForUpload() is now in utils.js (shared utility)
    ...
    try {
        debugLog(`📤 Processing upload: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`);
        ...
        debugLog(`✅ Optimized: ${originalSize}MB → ${optimizedSize}MB (saved ${savings}%)`);
...
async function analyzeSketch() {
    ...
    if (isAnalyzing) {
        warnLog('⚠️  Analysis already in progress, ignoring duplicate request');
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                warnLog("Could not parse error JSON from backend", jsonError);
            }
...
async function translatePrompt() {
    // ✅ FIX: Prevent double-click
    if (isTranslating) {
        warnLog('⚠️  Translation already in progress, ignoring duplicate request');
        return;
    }
    ...
    try {
        ...
        errorLog('❌ Translation failed:', error);
        throw error;
    } finally {
...
async function generateRender() {
    ...
    // ✅ FIX: Prevent double-click
    if (isRendering) {
        warnLog('⚠️  Rendering already in progress, ignoring duplicate request');
        return;
    }
    ...
    try {
        ...
        debugLog('📝 Sending form_data_vi with user edits:');
        debugLog('   - Building type:', form_data_vi.building_type);
        debugLog('   - Facade style:', form_data_vi.facade_style);
        debugLog('   - Environment items:', form_data_vi.environment.length);
        debugLog('   - Lighting:', form_data_vi.technical_specs.lighting);
        ...
        if (currentReferenceImage) {
            ...
            debugLog('📎 Using reference image for style consistency');
        }
...
function setupReferenceImageUI() {
    debugLog('🔧 Setting up Reference Image UI...');
    ...
    if (styleSection) {
        debugLog('✅ Found style section, inserting reference section before it');
        formElement.insertBefore(referenceSection, styleSection);
    } else {
        debugLog('⚠️  Style section not found, appending to end of form');
        formElement.appendChild(referenceSection);
    }

    debugLog('✅ Reference section inserted into DOM');
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', () => {
            ...
            debugLog('🗑️ Reference cleared');
        });
    }

    debugLog('✅ Reference Image UI setup complete!');
}
...
async function openReferenceLibrary() {
    try {
        ...
        errorLog('❌ Failed to load reference library:', error);
        showError('renderError', 'Không thể tải thư viện reference. Vui lòng kiểm tra backend.'); // ⭐ FIXED: No more alert()
    }
}
...
async function selectReferenceFromLibrary(imageId) {
    try {
        ...
        debugLog('✅ Reference selected from library:', imageId);
        showSuccess('renderSuccess', '✅ Đã chọn reference từ thư viện!'); // ⭐ FIXED: No more alert()
        
    } catch (error) {
        errorLog('❌ Reference download failed:', error);
...
function addUseAsReferenceButton() {
    ...
    btn.addEventListener('click', () => {
        ...
        debugLog('📎 Current render saved as reference');
    });
...
async function applyInpainting() {
    ...
    // ✅ FIX: Prevent double-click
    if (isInpainting) {
        warnLog('⚠️  Inpainting already in progress, ignoring duplicate request');
        return;
    }
    ...
    try {
        ...
        debugLog('✅ Inpainting complete');
        
        // Reset inpaint form
        document.getElementById('inpaintInstruction').value = '';
        
    } catch (error) {
        errorLog('❌ Inpainting failed:', error);
...
function setupPlanningMode() {
    debugLog('🔧 Setting up Planning Mode...');
    ...
    if (!buildingBtn || !planningBtn) {
        warnLog('⚠️  Mode selector buttons not found');
        return;
    }
...
function addLotDescription() {
    ...
    debugLog(`✅ Added lot description card #${lotNumber}`);
}
...
async function generatePlanningRender() {
    ...
    if (isPlanningRendering) {
        warnLog('⚠️  Planning render already in progress');
        return;
    }
    ...
    try {
        ...
        debugLog('✅ Planning render complete');
...
function displayPlanningRender(base64Data, mimeType) {
    ...
    debugLog('✅ Planning render displayed');
}
...
function downloadPlanningImage(base64Data) {
    try {
        ...
        debugLog('✅ Planning image downloaded');
...
// ============== END ==============
debugLog('📦 Script v3.2 loaded successfully - Planning Mode added! 🎉');